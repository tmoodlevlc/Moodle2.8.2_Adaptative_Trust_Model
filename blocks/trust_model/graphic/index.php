...
<html>
    <head>
		...
		<link rel="stylesheet"href="lib/dist/vis.css"/>
		<!--Booststrab-->
    ...
    <body>
		...
		$lstParticipantes= $DB->get_records_sql("SELECT * FROM 
												{role_assignments} 
												WHERE contextid = ? AND (roleid = ? OR roleid = ? OR roleid = ? OR roleid = ? OR roleid = ? ) 
												GROUP BY userid", 
												array($contexto->id, 
												$roleStudent->id, 
												$roleTeacher->id, 
												$roleTeacherSinEdition->id, 
												$roleManager->id, 
		...
		foreach($lstParticipantes as $participante){
			...
			$sqlParticipante= 	"SELECT  user_receptor, action, SUM(cont) as sum FROM 
									(SELECT posts_user as user_receptor, action, COUNT(*) as cont 
									FROM {trust_f1w1_history_forum} 
									WHERE course_id = $course_id AND posts_user=$participante->userid 
									GROUP BY user_receptor, action
									
									UNION ALL
									SELECT attempts_user as user_receptor, action, COUNT(*) as cont 
									FROM {trust_f1w1_history_quiz}
									WHERE course_id = $course_id AND attempts_user=$participante->userid 
									GROUP BY user_receptor, action 
									...
									WHERE course_id = $course_id AND (comments_user=$participante->userid OR assing_user=$participante->userid) 
									GROUP BY user_receptor, action 
									 
									UNION ALL
									SELECT book_user as user_receptor, action, COUNT(*) as cont 
									FROM {trust_f1w1_history_book}
									WHERE course_id = $course_id AND book_user=$participante->userid 
									GROUP BY user_receptor, action
									 
									UNION ALL
									SELECT file_user as user_receptor, action, COUNT(*) as cont 
									FROM {trust_f1w1_history_file}
									WHERE course_id = $course_id AND file_user=$participante->userid 
									GROUP BY user_receptor, action

									UNION ALL
									SELECT folder_file_user as user_receptor, action, COUNT(*) as cont 
									FROM {trust_f1w1_history_folder}
									WHERE course_id = $course_id AND folder_file_user=$participante->userid 
									GROUP BY user_receptor, action
									 
									UNION ALL
									SELECT page_user as user_receptor, action, COUNT(*) as cont 
									FROM {trust_f1w1_history_page}
									WHERE course_id = $course_id AND page_user=$participante->userid 
									GROUP BY user_receptor, action 

									UNION ALL
									SELECT url_user as user_receptor, action, COUNT(*) as cont 
									FROM {trust_f1w1_history_url}
									WHERE course_id = $course_id AND url_user=$participante->userid 
									GROUP BY user_receptor, action) 
		...
		<script type="text/javascript">		
            ...
            for(var i=0;i<arrayJSnodos.length;i++){
                if(arrayJSnodos[i].id === userActu){                                    
                    nodes.push({id: arrayJSnodos[i].id, 
								label: arrayJSnodos[i].firstname , 
								title: 'Nombres: '+arrayJSnodos[i].firstname+'<br>' + 
									   'Numero de (Me gusta): '+arrayJSnodos[i].like+'<br>'+ 
									   'Numero de (No me gusta): '+arrayJSnodos[i].not_like+'<br>'+
									   'Confianza: '+arrayJSnodos[i].trust+'<br>'+
									   'Confianza Externa: '+arrayJSnodos[i].trust_external, 
								group:'UseraActual'});
                }else{
                    nodes.push({id: arrayJSnodos[i].id, 
							    label: arrayJSnodos[i].firstname, 
            ...
            for(var i=0;i<arrayJSenlaces.length;i++){
                edges.push({from: arrayJSenlaces[i].user_emisor, 
		...
		for(var i=0;i<arrayLineaTiempo.length;i++){
			...
			item.label={content: arrayLineaTiempo[i].category+' ('+Number(arrayLineaTiempo[i].promedio).toFixed(2)+')'}
