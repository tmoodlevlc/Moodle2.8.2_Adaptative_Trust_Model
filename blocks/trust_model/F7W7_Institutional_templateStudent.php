...
if($opc=='save'){
		...
		$lst_questions  =  $DB -> get_records('trust_f7w7_t_questions',  array ('category'=>1, 'id_categories'=>$t_inst->category_course));
		...
		$t_answer= save_answers_total_f7w7(1,$t_inst->id,$u, $teacher, $value);
		...
		foreach($lst_questions as $q){
			...
			save_answers_student_f7w7($u, $teacher, $t_inst->id, $q->id, $q->type, $resp, $t_answer);
		}
		
		redirect(new moodle_url('/blocks/trust_model/F7W7_Institutional_templateStudent_lstProfesores.php',array('c' => $c, 'u' => $u, 'cat' => $cat)));
}else{
	...
	//Obtener las preguntas de la categoria
	$lst_questions =  $DB -> get_records('trust_f7w7_t_questions',  array ('category'=>1, 'id_categories'=>$categoria->category_course));
	$t = new html_table();
	$cont=0;
	if($lst_questions){
		...
		$form = '<div>';
		$form  .= '<form method="post" action="'.$CFG->wwwroot.'/blocks/trust_model/F7W7_Institutional_templateStudent.php?opc=save&c='.$c.'&u='.$u.'&teacher='.$teacher.'&cat='.$cat.'">';
		$form  .= $t;
