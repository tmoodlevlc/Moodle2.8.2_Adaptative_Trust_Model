...
if($opc=='save'){
		...
		$lst_questions  =  $DB -> get_records('trust_f7w7_t_questions',  array ('category'=>4, 'id_categories'=>$t_inst->category_course));
		...
		$t_answer= save_answers_total_f7w7(4,$t_inst->id,$u, $teacher, $value);
		...
		if($t_answer){
			foreach($lst_questions as $q){
				...
				save_answers_pairs_f7w7($u, $teacher, $t_inst->id, $q->id, $q->type, $resp, $t_answer);
			}
		}
		
		redirect(new moodle_url('/blocks/trust_model/F7W7_Institutional_templatePairs_lstProfesores.php',array('c' => $c, 'u' => $u, 'cat' => $cat, 'categoria' => $categoria, 'subcategoria' => $subcategoria)));
}else{
	...
	if($lst_questions){
		...
		$form = '<div>';
		$form  .= '<form method="post" action="'.$CFG->wwwroot.'/blocks/trust_model/F7W7_Institutional_templatePairs.php?opc=save&c='.$c.'&u='.$u.'&cat='.$cat.'&teacher='.$teacher.'&categoria='.$categoria.'&subcategoria='.$subcategoria.'">';
		$form  .= $t;
