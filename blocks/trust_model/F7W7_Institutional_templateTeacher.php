...
if($opc=='save'){
		...
		$lst_questions  =  $DB -> get_records('trust_f7w7_t_questions',  array ('category'=>2, 'id_categories'=>$t_inst->category_course));
		...
		$t_answer= save_answers_total_f7w7(2,$t_inst->id,$u, $u, $value);
		...
		foreach($lst_questions as $q){
			...
			save_answers_teacher_f7w7($u, $t_inst->id, $q->id, $q->type, $resp, $t_answer);
		}
	...
	if($teacher_validado){
		...
		if($lst_questions){
			...
			$form = '<div>';
			$form  .= '<form method="post" action="'.$CFG->wwwroot.'/blocks/trust_model/F7W7_Institutional_templateTeacher.php?opc=save&c='.$c.'&u='.$u.'&cat='.$cat.'">';
			$form  .= $t;
